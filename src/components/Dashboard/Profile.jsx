...
const Profile = ({mode, cardText, secColor}) => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (!token) return;
        const res = await axios.get(`${BACKEND_URL}/api/auth/getuser`, {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await axios.put(`${BACKEND_URL}/api/auth/updateuser`, form, {
        headers: {
          'auth-token': token,
  ...
  const handleDeleteImage = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.put(`${BACKEND_URL}/api/auth/delete-profile-image`, {}, {
        headers: { 'auth-token': token }
      });
      setUser(res.data.updatedUser);
              ...
              {products.map((product, idx) => (
                <div key={product._id || idx} className="col">
                  <div className="card h-100 shadow-sm border-0 position-relative">
                    {product.image?.[0] ? (
                      <img
                        src={`${BACKEND_URL}/uploads/${product.image[0]}`}
                        alt={product.title}
